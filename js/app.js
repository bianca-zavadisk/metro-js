...
async function runAnalysis() {
    ...
    try {
        const results = await computeHausdorff(state.meshA, state.meshB, numSamples, onProgress);
...
function renderResults(r) {
    ...
    <div class="metric-card">
      <div class="metric-name">Hausdorff Distance</div>
      <div class="metric-val accent">${fmt(r.hausdorff)}</div>
      <div class="metric-desc">
          max(E(S1,S2), E(S2,S1)) symmetric surface distance
      </div>
    </div>

    <div class="metric-card">
      <div class="metric-name">Reference → Simplified</div>
      <div class="metric-val accent">
          ${fmt(r.forwardError)}
      </div>
    </div>

    <div class="metric-card">
      <div class="metric-name">Simplified → Reference</div>
      <div class="metric-val accent">
          ${fmt(r.backwardError)}
      </div>
    </div>
    ...
    <div class="metric-card">
      <div class="metric-name">E+ (Expansion)</div>
      <div class="metric-val pos">${fmt(r.Eplus)}</div>
      <div class="metric-desc">max positive signed error · ${pct(r.positiveCount)} of samples</div>
    </div>
    <div class="metric-card">
      <div class="metric-name">E− (Contraction)</div>
      <div class="metric-val neg">${fmt(r.Eminus)}</div>
      <div class="metric-desc">max negative signed error · ${pct(r.negativeCount)} of samples</div>
    </div>
    <div class="metric-card">
      <div class="metric-name">Mean E+</div>
      <div class="metric-val pos">${fmt(r.meanPos)}</div>
      <div class="metric-desc">average over positive-sign samples</div>
    </div>
    <div class="metric-card">
      <div class="metric-name">Mean E−</div>
      <div class="metric-val neg">${fmt(r.meanNeg)}</div>
      <div class="metric-desc">average over negative-sign samples</div>
    </div>
    <div class="metric-card">
      <div class="metric-name">RMS E+</div>
      <div class="metric-val pos">${fmt(r.rmsPos)}</div>
      <div class="metric-desc">root mean square, positive samples</div>
    </div>
    <div class="metric-card">
      <div class="metric-name">RMS E−</div>
      <div class="metric-val neg">${fmt(r.rmsNeg)}</div>
      <div class="metric-desc">root mean square, negative samples</div>
    </div>
  `;
